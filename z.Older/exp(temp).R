...
colnames(kitsin)[c(22:26,29:30, 31:36)]<-c("Row_Stem_Density", "Height", "LAI", "Yield", "Lodging_Score", "Elevation", "VIS_Reflectance","NIR_Reflectance", "Flood_Affected", "Light_at_50","Fit_Type","Proportion_Saturated_Sun","Curvefit_Steepness" )
#Give this better names
library(jtools, broom)
sinkmodel<-(lm(Yield~Row_Stem_Density+Height+LAI+Flood_Affected+Lodging_Score+Elevation+VIS_Reflectance+NIR_Reflectance+Light_at_50+Fit_Type+Proportion_Saturated_Sun+Curvefit_Steepness, data=kitsin)) # [currently problems with those columns]
summary(sinkmodel)
summ(sinkmodel)
plot_summs(sinkmodel, scale=TRUE, colors = "#7B883F")

library(fastDummies)
kitsin.want<-kitsin[,c(22:26,29:30, 31:36)]; #:33, 35:
kitsin.dum<-dummy_cols(kitsin.want)[14:17]
kitsin.nums<-kitsin.want[,c(1:8, 10, 12:13)]
