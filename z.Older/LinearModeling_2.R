...
sinkmodel.sat<-lm(Yield~DSM_p+NDVI_p+LAI+Proportion_Saturated_Sun+Light_at_50+Interception_efficiency+Row_Stem_Density+Flood_Affected+Lodging_Score+Fit_Type+Curvefit_Steepness, data=na.omit(subset(kitsin, Interception_efficiency>0)))

sinkmodel.rate<-lm(Yield~DSM_p+GC_slp+DSM_slp+NDVI_p+LAI+Proportion_Saturated_Sun+Light_at_50+Interception_efficiency+Row_Stem_Density+Flood_Affected+Lodging_Score+Fit_Type+Curvefit_Steepness, data=na.omit(subset(kitsin, Interception_efficiency>0)))
  
sinkmodel.og<-lm(Yield~Height+LAI+Proportion_Saturated_Sun+Light_at_50+Interception_efficiency+NDVI+Row_Stem_Density+Flood_Affected+Lodging_Score+Fit_Type+Curvefit_Steepness, data=na.omit(subset(kitsin, Interception_efficiency>0)))

sinkmodel<-lm(Yield~Height+NDVI_p+LAI+Proportion_Saturated_Sun+Light_at_50+Interception_efficiency+Row_Stem_Density+Flood_Affected+Lodging_Score+Fit_Type+Curvefit_Steepness, data=na.omit(subset(kitsin, Interception_efficiency>0)))


#sinkmodel<-(lm(Yield~Height+LAI+Flood_Affected+Lodging_Score+Elevation+NDVI_p+Interception_efficiency+Row_Stem_Density+Fit_Type+Light_at_50+Proportion_Saturated_Sun+Curvefit_Steepness+DSM_slp+GC_slp, data=na.omit(subset(kitsin, Interception_efficiency>0.97))))# Fit type currently excluded # Light_at_50+Fit_Type+Proportion_Saturated_Sun+Curvefit_Steepness[currently problems with those columns]
summary(sinkmodel)
summ(sinkmodel)
plot_summs(sinkmodel, scale=TRUE, colors="forest green")#colors = "#7B883F")

# submodel<-(lm(Yield~Height+LAI+Row_Stem_Density+Flood_Affected+Lodging_Score2+Elevation+NDVI+Interception_efficiency+Row_Stem_Density, data=kitsin)) # Light_at_50+Fit_Type+Proportion_Saturated_Sun+Curvefit_Steepness[currently problems with those columns]
# plot_summs(submodel, scale=TRUE, colors="forest green")#colors = "#7B883F")
# summary(submodel)
# summ(submodel)

step(sinkmodel)
selectmodel<-step(sinkmodel.sat)
#lodging score #1
#selectmodel<-lm(formula = Yield ~ Row_Stem_Density + Height + LAI + Flood_Affected + 
                     #Lodging_Score + Elevation + NDVI_p + 
                     #Proportion_Saturated_Sun+GC_slp+DSM_slp, data = kitsin)
#lodging score #2
# selectmodel<-lm(formula = Yield ~ Height + LAI + Flood_Affected + Lodging_Score2 + Elevation + 
...
#Standardized
# sinkmodel.d<-(lm(Yield~Row_Stem_Density+Height+LAI+Flood_Affected_1+Lodging_Score2+Elevation+NDVI+Interception_efficiency+Light_at_50+Proportion_Saturated_Sun+Fit_Type_L+Curvefit_Steepness, data=kitsin.std)) # Light_at_50+Fit_Type+Proportion_Saturated_Sun+Curvefit_Steepness[currently problems with those columns]
# summary(sinkmodel.d)
# summ(sinkmodel.d)
...
summary(selectmodel.chk)
summ(selectmodel.chk)
plot_summs(selectmodel.chk, scale=TRUE, colors="forest green")#colors = "#7B883F")
