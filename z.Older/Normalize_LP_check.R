...
dat.c<-dat.check[,which(colnames(dat.check)%in%ppf.names)]

height.dat<-cliplp.dat<-data.frame(matrix(nrow=nrow(dat.c), ncol=6)) #Two df's of same size



for (i in 1:nrow(dat.check)){ #(i in (c(1:960)))
...
plotsamp<-sample(1:nrow(dat.check), 27); #plotsamp<-c(211,226)
#par(mfrow=c(3,3))
for(i in 1:nrow(dat.check)){ #1:960 for all
