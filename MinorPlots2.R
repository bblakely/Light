...
lodge.sinkmodel<-(lm(Lodging_Score~Height+LAI+Flood_Affected_1+Row_Stem_Density+Elevation+NDVI+Interception_efficiency+Light_at_50+Proportion_Saturated_Sun+Curvefit_Steepness, data=kitsin.std)) # Light_at_50+Fit_Type+Proportion_Saturated_Sun+Curvefit_Steepness[currently problems with those columns]
...
lodge.ind<-rep(0, nrow(kitsin));lodge.ind[which(kitsin$Lodging_Score2!=0)]<-1
boxplot(kitsin$Height~lodge.ind)

lodge.agg<-aggregate(Height~Lodging_Score2, data=kitsin, FUN='mean')
plot(lodge.agg, ylim=c(quantile(kitsin$Height, c(0.25,0.75)))); abline(h=mean(kitsin$Height))

vioplot(Height~Lodging_Score2, data=kitsin)
...
vioplot(kitsin$Lodging_Score2~test, names=paste(splits[1:8], '-', splits[2:9]),
...
dev.copy(png,'plots/Fig5_bcd.png', width=320, height=500)
dev.off()
...
ht<-ggplot(kitsin.want) +
  aes(x = Height) +
  geom_histogram(bins=30, color = "black", lwd=1) +
  labs(x = "Height (m)", y = "") +
  theme_minimal() +
  theme(axis.title.x = element_text(size = 12L)
  )

lai<-ggplot(kitsin.want) +
  aes(x = LAI) +
  geom_histogram(bins=30, color = "black", lwd=1) +
  labs(x = "LAI", y = "") +
  theme_minimal() +
...
dev.copy(png, "plots/histograms", x=750,y= )
