...
for(i in (1:nrow(dat.c))){
  ...
  if(any(atten>pres)){ #Breaks this curve somehow
...
#Goals:
#All plots range 0-1, i.e. bottom of plant to top of plant
#New algorithm to score with scaled sunlight


# want y value of next point up to be set at the x value of the actual height
...
#points(dat.rel$PPF2_Avg~dat.lp$lai, ylim=c(0,1), pch=19, col='blue')
#points(dat.rel$PPF3_Avg~dat.lp$lai, ylim=c(0,1), pch=19, col='red')
...
#
#Unscaled (by height) version. Light still scaled

lai.subset<-cbind(dat.rel[lai.ind,], dat.lp$lai[lai.ind],  dat.lp$height[lai.ind])
height.subset<-height.dat[lai.ind,]
colnames(lai.subset)[7:8]<-c('lai', 'height')

#Choice of heights for plotting
height<-heights/39.4
